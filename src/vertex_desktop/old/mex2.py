...
from PyQt6.QtGui import QFont, QTextCursor, QPalette, QColor, QIcon, QPixmap, QPainter, QLinearGradient, QTextCharFormat, QTextBlockFormat
...
class MarkdownFormatter:
    """Formats markdown text with Confluence-style code blocks"""
    
    @staticmethod
    def format_response(text):
        """Format the response with enhanced markdown and code block styling"""
        if not text:
            return text
            
        # Process the text to add Confluence-style formatting
        formatted_text = text
        
        # Handle code blocks with language detection
        formatted_text = MarkdownFormatter.format_code_blocks(formatted_text)
        
        # Format inline code
        formatted_text = MarkdownFormatter.format_inline_code(formatted_text)
        
        # Add other markdown formatting
        formatted_text = MarkdownFormatter.format_headers(formatted_text)
        formatted_text = MarkdownFormatter.format_lists(formatted_text)
        formatted_text = MarkdownFormatter.format_emphasis(formatted_text)
        
        return formatted_text
    
    @staticmethod
    def format_code_blocks(text):
        """Format code blocks with Confluence-style wrapper"""
        ...
        def replace_code_block(match):
            ...
            # Create Confluence-style code block
            confluence_block = f"""
<div class="code-block-wrapper">
    <div class="code-block-header">
        <span class="code-language">{language}</span>
        <button class="code-copy-btn" onclick="copyCode(this)">📋 Copy</button>
        <button class="code-collapse-btn" onclick="toggleCode(this)">🔽 Collapse</button>
    </div>
    <pre class="code-block" data-language="{language}"><code>{code}</code></pre>
</div>"""
            return confluence_block
    ...
    def format_headers(text):
        ...
        for i in range(6, 0, -1):
            pattern = r'^' + '#' * i + r'\s+(.+)$'
            replacement = f'<h{i} class="markdown-header">\\1</h{i}>'
    ...
    def format_lists(text):
        ...
        # Unordered lists
        text = re.sub(r'^\*\s+(.+)$', r'<li class="ul-item">\1</li>', text, flags=re.MULTILINE)
        text = re.sub(r'^\-\s+(.+)$', r'<li class="ul-item">\1</li>', text, flags=re.MULTILINE)
        
        # Ordered lists
        text = re.sub(r'^\d+\.\s+(.+)$', r'<li class="ol-item">\1</li>', text, flags=re.MULTILINE)
    ...
    @staticmethod
    def get_code_block_styles(colors, font_size):
        """Get CSS styles for code blocks"""
        return f"""
        <style>
        .code-block-wrapper {{
            margin: 10px 0;
        ...
        .code-block-header {{
            ...
            align-items: center;
            padding: 8px 12px;
            background: linear-gradient(to right, {colors['primary']}22, {colors['secondary']}22);
        ...
        .code-block {{
            margin: 0;
            padding: 12px;
            background-color: {colors['background'] if colors == theme_manager.dark_colors else '#F9FAFB'};
            overflow-x: auto;
        ...
        h1.markdown-header {{ font-size: {font_size + 10}px; }}
        h2.markdown-header {{ font-size: {font_size + 7}px; }}
        h3.markdown-header {{ font-size: {font_size + 4}px; }}
        h4.markdown-header {{ font-size: {font_size + 2}px; }}
        h5.markdown-header {{ font-size: {font_size}px; }}
        h6.markdown-header {{ font-size: {font_size - 1}px; }}
...
# Enhanced QTextBrowser for formatted output
class FormattedResponseBrowser(QTextBrowser):
    """Custom text browser for displaying formatted markdown responses"""
    ...
    def set_formatted_text(self, text, colors, font_size):
        """Set text with markdown formatting"""
        if not text:
            self.clear()
            return
            
        # Get styles
        styles = MarkdownFormatter.get_code_block_styles(colors, font_size)
        ...
        <body style="font-family: 'SF Pro Text', system-ui, -apple-system, sans-serif; 
...
class AboutDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        <ul>
            <li><b>Markdown Formatting:</b> Automatic formatting of LLM responses</li>
            <li><b>Code Blocks:</b> Confluence-style code blocks with syntax highlighting</li>
            <li><b>Collapsible Content:</b> Expand/collapse code sections</li>
            <li><b>Copy Code:</b> One-click code copying from formatted blocks</li>
            <li><b>Line Wrapping:</b> Automatic line wrapping for long code lines</li>
        </ul>
...
class QueryTab(QWidget):
    """Individual query tab widget with enhanced design and formatted output"""
    ...
    def init_ui(self):
        ...
        # Response text - Use FormattedResponseBrowser for rich text formatting
        self.response_edit = FormattedResponseBrowser()
    ...
    def save_response(self):
        ...
        if self.show_raw_json_checkbox.isChecked():
            ...
            file_filter = "Text Files (*.txt);;Markdown Files (*.md);;JSON Files (*.json);;All Files (*.*)"
        ...
        if file_path:
            try:
                ...
                # Add metadata header for non-JSON files
...
# Format: {'Raw JSON' if self.show_raw_json_checkbox.isChecked() else 'Formatted Text'}
{"="*50}

"""
                    content_to_save = metadata + content_to_save
    ...
    def update_font_sizes(self, size):
        ...
        # Update the formatted response if it exists
        if self.parsed_response:
            self.display_formatted_response(self.parsed_response)
    ...
    def update_theme(self):
        ...
        # Update the formatted response if it exists
        if self.parsed_response and not self.show_raw_json_checkbox.isChecked():
            self.display_formatted_response(self.parsed_response)

    def display_formatted_response(self, text):
        """Display the response with proper formatting"""
        if isinstance(self.response_edit, FormattedResponseBrowser):
            self.response_edit.set_formatted_text(text, COLORS, font_manager.base_size)
        else:
            self.response_edit.setPlainText(text)

    def toggle_response_format(self):
        """Toggle between raw JSON and formatted text display"""
